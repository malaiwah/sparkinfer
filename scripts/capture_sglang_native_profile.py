...
# ---------------------------------------------------------------------------
# Bounded HTTP connection (deadline on DNS/connect/TLS/send/getresponse)
# ---------------------------------------------------------------------------
...
def http_json(
    ...
    *,
    ...
    status code and bounded body rather than raising HTTPError.
    """
...
def main() -> None:
    ...
    try:
        ...
        _secure_write_text(
            "start_profile.json",
            dir_fd,
            json.dumps(profile_body, indent=2) + "\n",
        )
        ...
        try:
            ...
            _secure_write_bytes("profile.response", dir_fd, body_bytes)
            if status != 200:
                raise RuntimeError(
        ...
        manifest = {
            ...
            "notes": [
                ...
                "For speculative requests, native SGLang profiling buckets "
                "TARGET_VERIFY and DRAFT_EXTEND under the prefill-family stage.",
                "On older SGLang profiler paths, profile_stages is ignored and "
                "profiling still buckets into prefill-family vs decode.",
            ],
        }
        _secure_write_text(
            "manifest.json",
