...
def test_dense_export_refuses_existing_destination_without_mutation(
...
def test_hard_linked_auxiliary_file_rejected(tmp_path: pathlib.Path) -> None:
    src = tmp_path / "src"
    src.mkdir()
    original = tmp_path / "shared-tokenizer.json"
    original.write_bytes(SENTINEL)
    os.link(original, src / "tokenizer.json")
    dst = tmp_path / "dst"
    dst.mkdir()

    with pytest.raises(UnsafeAuxiliaryFileError, match="hard-linked"):
        _copy_aux_files(src, dst)

    assert not (dst / "tokenizer.json").exists()


def test_dense_export_dry_run_creates_no_destination(
    tmp_path: pathlib.Path,
) -> None:
    src = tmp_path / "d"
    _fake_dense(src, layers=1, k=64, n=64)
    ...
    assert report.out_dir == str(out)
    assert not out.exists()


def test_atomic_publish_never_replaces_existing_name(
    tmp_path: pathlib.Path,
) -> None:
    import b12x.quantization.mxfp6.fp6_safetensors_export as mod
    ...
    try:
        with pytest.raises(FileExistsError):
            mod._rename_noreplace(parent_fd, source.name, destination.name)
    finally:
        os.close(parent_fd)
...
def test_staging_creation_failure_closes_parent_fd(
    tmp_path: pathlib.Path,
    ...
    monkeypatch.setattr(mod.tempfile, "mkdtemp", fail_mkdtemp)
    with (
        pytest.raises(OSError, match="injected staging failure"),
        mod._staged_output_directory(tmp_path / "output"),
    ):
        pytest.fail("staging context yielded")
