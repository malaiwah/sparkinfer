...
def test_checkpoint_dense_loader_rejects_scale_before_swizzle(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    tensors = {
        "linear.weight": torch.zeros((64, 96), dtype=torch.uint8),
        "linear.weight_scale": torch.zeros((64, 3), dtype=torch.uint8),
        "linear.weight_scale_2": torch.ones(1),
    }
    monkeypatch.setattr(
        "b12x.quantization.mxfp6.fp6_safetensors_load.swizzle_block_scale",
        lambda *_args: pytest.fail("malformed scale reached swizzle"),
    )
    with pytest.raises(ValueError, match="weight_scale shape"):
        load_fp6_dense_weight_from_safetensors(
            tensors.__getitem__, "linear", device="cpu"
        )


def test_checkpoint_dense_loader_rejects_invalid_packed_width() -> None:
    tensors = {
        "linear.weight": torch.zeros((64, 95), dtype=torch.uint8),
        "linear.weight_scale": torch.zeros((64, 4), dtype=torch.uint8),
        "linear.weight_scale_2": torch.ones(1),
    }
    with pytest.raises(ValueError, match="invalid packed FP6 shape"):
        load_fp6_dense_weight_from_safetensors(
            tensors.__getitem__, "linear", device="cpu"
        )


def test_checkpoint_moe_loader_rejects_late_expert_before_cat(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    tensors: dict[str, torch.Tensor] = {}
    for expert in range(2):
        base = f"layer.experts.{expert}"
        tensors[f"{base}.gate_proj.weight"] = torch.zeros(
            (64, 96), dtype=torch.uint8
        )
        tensors[f"{base}.gate_proj.weight_scale"] = torch.zeros(
            (64, 4), dtype=torch.uint8
        )
        tensors[f"{base}.up_proj.weight"] = torch.zeros(
            (64, 96), dtype=torch.uint8
        )
        tensors[f"{base}.up_proj.weight_scale"] = torch.zeros(
            (64, 4), dtype=torch.uint8
        )
        tensors[f"{base}.down_proj.weight"] = torch.zeros(
            (128, 48), dtype=torch.uint8
        )
        tensors[f"{base}.down_proj.weight_scale"] = torch.zeros(
            (128, 2), dtype=torch.uint8
        )
        for proj in ("gate_proj", "up_proj", "down_proj"):
            tensors[f"{base}.{proj}.weight_scale_2"] = torch.ones(1)
            tensors[f"{base}.{proj}.input_scale"] = torch.ones(1)
    tensors["layer.experts.1.up_proj.weight_scale"] = torch.zeros(
        (64, 3), dtype=torch.uint8
    )
    monkeypatch.setattr(
        torch,
        "cat",
        lambda *_args, **_kwargs: pytest.fail("malformed expert reached cat"),
    )
    with pytest.raises(ValueError, match="weight_scale shape"):
        load_fp6_moe_weights_from_safetensors(
            tensors.__getitem__,
...
def test_checkpoint_moe_loader_rejects_nonfinite_global_scale(
    projection: str,
) -> None:
    tensors: dict[str, torch.Tensor] = {
        "layer.experts.0.gate_proj.weight": torch.zeros(
            (64, 96), dtype=torch.uint8
        ),
        "layer.experts.0.gate_proj.weight_scale": torch.zeros(
            (64, 4), dtype=torch.uint8
        ),
        "layer.experts.0.up_proj.weight": torch.zeros(
            (64, 96), dtype=torch.uint8
        ),
        "layer.experts.0.up_proj.weight_scale": torch.zeros(
            (64, 4), dtype=torch.uint8
        ),
        "layer.experts.0.down_proj.weight": torch.zeros(
            (128, 48), dtype=torch.uint8
        ),
        "layer.experts.0.down_proj.weight_scale": torch.zeros(
            (128, 2), dtype=torch.uint8
        ),
        "layer.experts.0.gate_proj.weight_scale_2": torch.ones(1),
        "layer.experts.0.down_proj.weight_scale_2": torch.ones(1),
        "layer.experts.0.gate_proj.input_scale": torch.ones(1),
        "layer.experts.0.down_proj.input_scale": torch.ones(1),
        "layer.experts.0.up_proj.weight_scale_2": torch.ones(1),
        "layer.experts.0.up_proj.input_scale": torch.ones(1),
    }
    tensors[f"layer.experts.0.{projection}.weight_scale_2"] = torch.tensor(
        float("nan")
    )
    with pytest.raises(NotImplementedError, match="finite and unit"):
        load_fp6_moe_weights_from_safetensors(
            tensors.__getitem__,
            "layer",
            1,
...
def test_dataclass_rejects_invalid_global_scale_value(value: float) -> None:
    packed, scale, _ = _make_valid_tensors(128, 128)
    with pytest.raises(ValueError, match="finite positive"):
        FP6DenseWeight(
            packed=packed,
            scale_storage=scale,
            global_scale=torch.tensor([value], dtype=torch.float32),
            out_features=128,
            in_features=128,
            fmt="e2m3",
        )
...
def test_checkpoint_dense_loader_requires_unit_input_scale(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    tensors = {
        "linear.weight": torch.zeros((64, 96), dtype=torch.uint8),
        "linear.weight_scale": torch.zeros((64, 4), dtype=torch.uint8),
        "linear.weight_scale_2": torch.ones(1),
        "linear.input_scale": torch.tensor([2.0]),
    }
    monkeypatch.setattr(
        "b12x.quantization.mxfp6.fp6_safetensors_load.swizzle_block_scale",
        lambda *_args: pytest.fail("invalid input scale reached swizzle"),
    )
    with pytest.raises(NotImplementedError, match="finite and unit"):
        load_fp6_dense_weight_from_safetensors(
            tensors.__getitem__, "linear", device="cpu"
...
def test_checkpoint_config_identity_is_explicit_and_fail_closed() -> None:
    valid = {
        ...
        "weight_format": "e2m3",
        "activation_format": "e3m2",
    }
    assert _source_format_from_config(valid) == "mxfp6_default"
    for key in tuple(valid):
        malformed = dict(valid)
        malformed.pop(key)
        with pytest.raises(ValueError, match="missing required"):
            _source_format_from_config(malformed)
    malformed = dict(valid, quant_algo="NVFP4")
