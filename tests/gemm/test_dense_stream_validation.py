...
def test_record_tensors_on_device_current_stream(monkeypatch) -> None:
    """Every non-None operand is recorded without constructing a driver wrapper."""
    import b12x._lib.utils as stream_utils
    ...
    monkeypatch.setattr(
        stream_utils.cuda,
        "CUstream",
        lambda _handle: pytest.fail("trace-visible helper constructed CUstream"),
    )
    launch_stream = record_tensors_on_current_stream(tensors, device)

    assert launch_stream is current
    assert recorded == [current, current]


@pytest.mark.skipif(not torch.cuda.is_available(), reason="CUDA required")
def test_record_tensors_is_fullgraph_traceable() -> None:
    """Dynamo can retain the allocator-lifetime primitive in a full graph."""
    device = torch.device("cuda", torch.cuda.current_device())

    def recorded_add_one(source: torch.Tensor) -> torch.Tensor:
        record_tensors_on_current_stream([source], source.device)
        return source + 1

    compiled = torch.compile(recorded_add_one, backend="eager", fullgraph=True)
    source = torch.arange(16, dtype=torch.float32, device=device)
    assert torch.equal(compiled(source), source + 1)
